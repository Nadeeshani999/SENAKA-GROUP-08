...
async function initTenderPage() {
  ...
  function renderView(rows) {
    ...
    const delayEl=document.getElementById('tender-delay-meters');
    if (delayEl) {
      const delayRows=rows.filter(r=>{
        const d=parseFloat(r['Delays (Expected or Actual)']??r['Delays']??'');
        return !isNaN(d);
      });
      if (!delayRows.length) {
        delayEl.innerHTML='<div style="color:rgba(255,255,255,.35);font-size:.75rem;padding:.5rem">No delay data available</div>';
      } else {
        // Inject delay-pulse CSS animation if not already present
        if (!document.getElementById('delay-meter-style')) {
          ...
          dst.textContent=`
            ...
            .delay-meter-card { text-align:center; min-width:148px; }
            .delay-meter-card.delayed { animation:delayPulse 1.3s ease-in-out infinite; }
          `;
          document.head.appendChild(dst);
        }

        // Find max abs delay for scale
        const allDelays=delayRows.map(r=>parseFloat(r['Delays (Expected or Actual)']??r['Delays']??0)||0);
        const maxAbs=Math.max(1,...allDelays.map(d=>Math.abs(d)));

        let hasDelayed=false;
        const delayedTenders=[];

        delayEl.innerHTML=delayRows.map((r,i)=>{
          ...
          const absVal=Math.abs(delayVal);
          // Fill pct capped at 100, relative to max in current set
          const fillPct=Math.min(100,(absVal/maxAbs)*100);
          ...
          const arrTip=`${ntx.toFixed(2)},${nty.toFixed(2)}`;
          const arrL=`${(bx-px).toFixed(2)},${(by+py).toFixed(2)}`;
          const arrR=`${(bx+px).toFixed(2)},${(by-py).toFixed(2)}`;
          ...
          const tabLabel=r._tabStatus==='On-Going'?'On-Going':'Completed';
          const statusLabel=isDelayed
            ? `⚠ +${Math.round(delayVal)}d DELAYED`
            : isAhead
              ? `✅ ${Math.round(delayVal)}d ahead`
              : `✔ On schedule`;

          return `<div class="delay-meter-card${isDelayed?' delayed':''}" title="${r['Tender No']||''} · ${tabLabel} · Delay: ${delayVal}d">
            <svg viewBox="0 0 120 72" width="148" height="88" style="overflow:visible">
              <defs>
                <linearGradient id="${gmId}" x1="0%" y1="0%" x2="100%" y2="0%">
              ...
              <path d="M ${arrTip} L ${arrL} L ${arrR} Z" fill="${col}" ${isDelayed?`filter="url(#dgl-${i})"`:''} style="filter:drop-shadow(0 0 ${isDelayed?5:2}px ${col})"/>
              <circle cx="${cx}" cy="${cy}" r="4" fill="${col}" stroke="rgba(0,0,0,.5)" stroke-width="1.5"/>
            ...
            <div style="font-size:.64rem;font-weight:700;color:${col};margin-top:-.2rem;letter-spacing:.4px">${statusLabel}</div>
            <div style="font-size:.62rem;color:rgba(255,255,255,.55);margin-top:.18rem;font-weight:600">${r['Tender No']||'—'}</div>
            <div style="font-size:.58rem;color:rgba(255,255,255,.35);margin-top:.1rem">${r['Site Name']||'—'} · <span style="color:${r._tabStatus==='On-Going'?C.blue:C.green};opacity:.8">${tabLabel}</span></div>
          </div>`;
        }).join('');

        // Toast + alarm if any delayed tenders
        if (hasDelayed) {
